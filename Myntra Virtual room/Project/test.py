...
root = Tk()
root.title('MYNTRA Trial room')
root.configure(background='violet red')
app=FullScreenApp(root)

top_frame = Frame(root, bg='#D12378', width=50, height=50, pady=3)
center = Frame(root, bg='violet red', width=50, height=40, padx=3, pady=3)
btm_frame = Frame(root, bg='#D12378', width=50, height=50, pady=3)
...
ctr_left = Label(center,bg='violet red', width=50, height=190)
ctr_mid = Label(center,bg='violet red',width=100, height=160, padx=0, pady=0)
...
for index in range(9):
    if len(PHOTOS[index]) > 0:
        for k,photo in enumerate(PHOTOS[index]):
            btn= Button(scrollable_body, highlightbackground='violet red', text=IMAGES[index][k].rsplit('/',1)[1].replace('.png','')[:-1],bg='violet red', image=photo, command = lambda index=index, k=k: put_sprite(index,k), compound=LEFT, width='300', height='200')
